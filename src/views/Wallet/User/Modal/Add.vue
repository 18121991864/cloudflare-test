...
<template>
    <a-modal v-model:open="visible" title="添加钱包账户" :confirm-loading="isLoading" @ok="handleOk">
        <a-form ref="formRef" :model="formState" name="basic" :label-col="{ span: 4 }" layout="vertical" autocomplete="off">
            <a-form-item label="钱包品牌" name="state" :rules="[{ required: true, message: '请选择' }]">
            ...
            <a-form-item label="代理商ID" name="remark" :rules="[{ required: true, message: '请输入' }]">
                <a-input v-model:value="formState.remark" placeholder="请输入" />
            </a-form-item>
            <a-form-item label="数量" name="remark" :rules="[{ required: true, message: '请输入' }]">
                <a-input v-model:value="formState.remark" placeholder="请输入" />
            </a-form-item>
        </a-form>
    </a-modal>
