<template>
    ...
    >
        ...
        <a-form ref="formRef" class="form-list" :label-col="{ span: 5 }" hide-required-mark :rules="formRules" :model="formState">
            <a-form-item name="info" label="弃用原因">
                <a-input v-model:value="formState.info" size="small" placeholder="弃用原因" />
            </a-form-item>
        </a-form>
    </a-modal>
</template>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const formRules = computed<Record<string, Rule[]>>(() => ({
            // 表单规则
            info: [{ required: true, trigger: 'blur', message: '请输入' }],
        }));

        const title = computed(() => `${props.type === 'add' ? '弃用' : '弃用'}地址${props.type === 'add' ? '' : `-${props.activeData?.id}`}`);
        ...
        const handleAddOrUpdate = () => {
            ...
            // @ts-ignore
            return api
                .fetchAddressDisuse({ ...formState, id: props.activeData?.id })
                .then(() => {
                    emit('onClose');
