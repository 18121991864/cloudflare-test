    ...
    <div class="table-container">
        ...
        >
            <template #bodyCell="{ column: { dataIndex }, record: { status, amlFrozen }, record }">
                ...
                <template v-else-if="dataIndex === 'AMLStatus'">
                ...
                <template v-else-if="dataIndex === 'operate'">
                    <a-button type="link" @click="handleAmlFrozen(record)"> 冻结关联订单 </a-button>
                </template>
            </template>
        </a-table>
...
const tableColumns: ComputedRef<TableColumnType[]> = computed(() => [
    ...
    { title: '冻结总金额 ', dataIndex: 'amlFrozen' },
...
const handleOnSubmit = async () => {
    ...
    api.getAddressList({
        ...
        .then(({ list, totalSize }: PromiseReturnType<typeof api.getAddressList>) => {
