...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const getList = ref<PromiseReturnType<typeof api.fetchGetCurrency>>(Object.create(null));
        const handleList = (open: boolean = true) => {
            if (loading.value || !open) return;
            loading.value = true;
            getList.value = [];
            api.fetchGetCurrency()
                .then((data: PromiseReturnType<typeof api.fetchGetCurrency>) => {
                    getList.value = data;
                })
                .finally(() => {
                    loading.value = false;
        ...
        const handelSelect = (value: string) => {
            const item = getList.value.find((x) => x.itemId === value);
