...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const handleList = (open: boolean = true) => {
            ...
            loading.value = true;
            getList.value = [];
            api.fetchDitchTransactionKoipyMessageTypeList()
                .then((data: PromiseReturnType<typeof api.fetchDitchTransactionKoipyMessageTypeList>) => {
                    getList.value = data;
                })
                .finally(() => {
                    loading.value = false;
