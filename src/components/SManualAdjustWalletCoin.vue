...
export default defineComponent({
    name: 'SManualAdjustWalletCoin',
    props: {
        modelValue: { type: String, default: '', required: true },
        pure: { type: Boolean, default: false },
        readOnly: { type: Boolean, default: false, required: false },
        disabled: { type: Boolean, default: false, required: false },
        businessType: { type: String, default: '', required: false },
    ...
    setup(props, context) {
        ...
        const getList = ref<PromiseReturnType<typeof api.fetchManualAdjustWalletGetCoin>>(Object.create(null));
        const handleList = (open: boolean = true) => {
            if (loading.value || !open) return;
            loading.value = true;
            getList.value = [];
            api.fetchManualAdjustWalletGetCoin({
                accountId: props.accountId,
            })
                .then((data: PromiseReturnType<typeof api.fetchManualAdjustWalletGetCoin>) => {
                    if (['3', '4', '5'].includes(props.businessType)) {
                        getList.value = data.filter((item) => {
                            return item.label === 'USDT';
                        });
                    } else {
                        getList.value = data;
                    }
