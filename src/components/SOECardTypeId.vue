...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const getList = ref<PromiseReturnType<typeof api.fetchManualAdjustCardGetEntryType>>(Object.create(null));
        const handleList = (open: boolean = true) => {
            if (loading.value || !open) return;
            loading.value = true;
            getList.value = [];
            api.fetchManualAdjustCardGetEntryType()
                .then((data: PromiseReturnType<typeof api.fetchManualAdjustCardGetEntryType>) => {
