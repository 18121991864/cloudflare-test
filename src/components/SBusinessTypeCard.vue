...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const getList = ref<PromiseReturnType<typeof api.fetchManualAdjustCardGetBusinessType>>(Object.create(null));
        const handleList = (open: boolean = true) => {
            if (loading.value || !open) return;
            loading.value = true;
            getList.value = [];
            api.fetchManualAdjustCardGetBusinessType()
                .then((data: PromiseReturnType<typeof api.fetchManualAdjustCardGetBusinessType>) => {
                    if (props.isRequestDitch === '1') {
                        getList.value = data.filter((item: any) => item.value === '1');
                    } else {
                        getList.value = data;
                    }
                })
