...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const handleList = (open: boolean = true) => {
            ...
            loading.value = true;
            getList.value = [];
            api.fetchSinoDitchTransactionMessageTypeList({ ditchCode })
                .then((data: PromiseReturnType<typeof api.fetchSinoDitchTransactionMessageTypeList>) => {
                    getList.value = data;
                })
                .finally(() => {
                    loading.value = false;
