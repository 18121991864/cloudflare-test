...
// 加密对应key
export const getCiphertext = async (str: string): Promise<string> => {
    const store = useEncryptionStore();
    const { userInfo, ivStr } = toRefs(store);
    if (!ivStr.value) {
        const config = await axios.get('/api/v1/sysConfig/pwdIv');
        store.setIvStr(config.data.data);
    }
    const key = userInfo.value.userId ? userInfo.value.userId : userInfo.value.account;
    return await encryptAESGCM(str, md5(`${key}sys-api`), ivStr.value);
};
