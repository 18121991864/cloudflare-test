# wallet-manage项目目录结构及模块介绍

## Vue 3 + TypeScript + Vite + tailwind
#### api文件夹存放的是接口文件,命名细则是根据后端接口地址来命名创建
    如：/api/v1/account, 则在文件中创建：account.ts文件
    多个重名的接口url,比如: /account, /account/user/, 
    可以在api文件夹下面以目录的形式拆分，变为：account目录，主入口: index.ts、user.ts
#### components目录是公共模块组件存放目录
#### assets静态资源目录
    images存放图片
...
#### views项目组件存放目录
     项目文件创建是以大模块为主文件目录，细分模块为辅的文件目录构造。例如，首页模块创建Home文件夹, Index.tsx为主入口，里面则对应细分如：Banner模块、Notify模块、Download模块 
